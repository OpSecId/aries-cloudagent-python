"""IssuerService for performing Data Integrity Proof signatures over JSON-LD formatted W3C VCs."""
...
class IssuerService:
    ...
    def __init__(self, profile: Profile):
        """Initialize the VC LD Issuer Service."""
    ...
    async def _get_suite_for_document(
        ...
        if not proof_type:
            raise IssuerServiceError("Proof type is required")
    ...
    async def verify_presentation(
        ...
        if not options.challenge:
            raise IssuerServiceError("Challenge is required for verifying a VP")
